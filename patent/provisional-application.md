...
**26.18 Per-Tile Micro-Pump and Distributed Pressure Boosting.** Each tile base may include a small booster pump (12V DC diaphragm or impeller type, approximately 2 GPM capacity, 60 PSI maximum, approximately 50W peak power draw) on the water supply path. The micro-pump re-pressurizes water as it passes through each tile, overcoming friction losses at the quick-connect fittings and internal passages. This distributed pumping architecture eliminates theoretical chain-length limitations on the water mesh: an arbitrarily long chain of tiles each re-pressurizes the flow, maintaining adequate pressure at any distance from a sub-channel injection point. For fire suppression applications requiring high flow (8+ GPM per head per NFPA 13), the central controller opens multiple parallel supply paths through the mesh simultaneously, with each path's chain of micro-pumps maintaining pressure independently.
...
**76.** The system of claim 55, wherein each tile base includes a micro-pump on the water supply path that re-pressurizes flow through the tile, enabling unlimited daisy-chain water distribution across the mesh, and wherein the central controller opens multiple parallel mesh paths simultaneously for fire suppression loads exceeding single-link flow capacity.
